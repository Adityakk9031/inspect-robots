...
def test_run_setup_options_suggest_non_bool_or_raising_falls_back(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    raising_slot = OptionSlot(
        arg="opt1",
        label="Opt1",
        default=False,
        suggest=lambda args: 1 / 0 == 0,
    )
    non_bool_slot = OptionSlot(
        arg="opt2",
        label="Opt2",
        default=False,
    ...
    class _Factory:
        OPTION_SLOTS: ClassVar[tuple[OptionSlot, ...]] = (raising_slot, non_bool_slot)
    ...
    assert result == 0
    prompt1 = next(p for p in prompts if "Opt1" in p)
    prompt2 = next(p for p in prompts if "Opt2" in p)
    assert "[y/N]" in prompt1
    assert "[y/N]" in prompt2
